...
func (j *JobManager) Start(ch *amqp.Channel) {
	...
	if err != nil {
		log.Fatalf("Error converting CONCURRENCY_WORKERS to int: %v", err.Error())
	}
...
func (j *JobManager) notify(jobJson []byte) error {
	...
	err := j.RabbitMQ.Notify(string(jobJson), "application/json", os.Getenv("RABBITMQ_NOTIFICATION_EXCHANGE"), os.Getenv("RABBITMQ_NOTIFICATION_ROUTING_KEY"))
